...
#transforming column date to date format

df1$Dia = gsub(x = df1$Dia, replacement = '', pattern =  "\\s\\d*[h]\\d*")

df1$Dia = dmy(df1$Dia)

#Creating column where the protest happened
...
p + geom_bar(fill = "#87CEFA") +  
  ...
  ggtitle('News about Protests overtime')
...
ggplot(puk, aes(Mes)) + 
  ...
  ylab(label = '# of news') +
  xlab(label = 'Year - Month') +
  ggtitle('Protests in United Kingdom')
